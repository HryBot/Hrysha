...
Host="hrybottest777.atlassian.net"
...
def Add_Is(Host, Login, Pas, Name, Desc, Ass):
    jiraa.Make_Issue(Host,Login,Pas,Ass,Name,Desc,0,0)
def Nazv_Proect():
    Ot=[]
...
def changeStatus(SENDER_ID, TEXT_FROM_MESS1, step):
    dict = {}
    if step == "0":
        dict.update({SENDER_ID: []})
        dict[SENDER_ID].append(TEXT_FROM_MESS1)
    ...
    with open("./Utils/queue.json", 'r') as function:
        if step == "1":
            dict.update({SENDER_ID: []})
            dict[SENDER_ID].append(TEXT_FROM_MESS1)
            print(dict[SENDER_ID])
            jiraa.Make_Done(Host, Login, Pas, dict[SENDER_ID], 'Done')
            dict.pop(str(SENDER_ID), None)
...
def adding(SENDER_ID, TEXT_FROM_MESS1, step):
    ...
    with open("./Utils/queue.json", 'r') as function:
        if step == "3":
            dict.update({SENDER_ID: []})
            dict[SENDER_ID].append(TEXT_FROM_MESS1)
            jiraa.Make_Issue(Host,Login,Pas,dict[SENDER_ID][2], dict[SENDER_ID][0], dict[SENDER_ID][1], dict[SENDER_ID][3], dict[SENDER_ID][4])
            dict.pop(str(SENDER_ID), None)
            funcFile = json.loads(function.read())
            funcFile.pop(str(SENDER_ID), None)
            open("./Utils/queue.json", 'w').write(json.dumps(funcFile))
    ...
    while True:
        ...
        if LAST_TIME_EVENT != xx and xxx != botID:
            ...
            if AuthCheck(SENDER_ID)==1:
                    ...
                    elif (TEXT_FROM_MESS1 == "Завершить"):
